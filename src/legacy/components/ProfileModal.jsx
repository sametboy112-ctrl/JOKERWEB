      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 pointer-events-none">
            <motion.section role="dialog" aria-modal="true" aria-labelledby="profile-title" className="pointer-events-auto w-full max-w-md max-h-[90vh] overflow-y-auto rounded-3xl border border-white/10 bg-[#0b0f19] shadow-2xl" initial={{ opacity: 0, y: 24, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: 0.98 }}>
              <div className="flex items-center justify-between border-b border-white/10 p-6">
                <div className="flex items-center gap-3"><FaUserCircle className="text-3xl text-red-400" /><div><h2 id="profile-title" className="text-lg font-bold text-white">My Profile</h2><p className="text-xs text-gray-500">Account and premium membership</p></div></div>
                <button type="button" onClick={onClose} aria-label="Close profile" className="rounded-full p-2 text-gray-400 hover:bg-white/10 hover:text-white"><FaTimes /></button>
              </div>
              <div className="space-y-5 p-6">
                <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4"><p className="text-base font-semibold text-white">{user?.user_metadata?.displayName || user?.user_metadata?.name || user?.email?.split('@')[0] || 'Member'}</p><p className="mt-1 break-all text-sm text-gray-400">{user?.email}</p><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-xl bg-black/20 p-3"><p className="text-2xl font-black text-white">{watchedCount}</p><p className="text-xs text-gray-500">Movies watched</p></div><div className="rounded-xl bg-black/20 p-3"><p className="text-2xl font-black text-amber-400">Free</p><p className="text-xs text-gray-500">Current plan</p></div></div></div>
