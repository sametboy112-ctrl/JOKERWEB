...
export function WatchlistProvider({ children }) {
  ...
  const toggleWatchlist = useCallback(async (item, onNeedAuth) => {
    ...
    const nextItem = { ...item, id, addedAt: new Date().toISOString() };

    // Update immediately; Firestore's snapshot confirms or corrects it later.
    ...
    setWatchlistItems((current) => (
      wasSaved
        ? current.filter((entry) => String(entry.mediaId) !== id)
        : [nextItem, ...current.filter((entry) => String(entry.mediaId) !== id)]
    ));
