...
function Series() {
  ...
  useEffect(() => {
    ...
    // Only migrate old query-string URLs. Watching pathname here lets the
    // outgoing page overwrite a click to another section before it unmounts.
    if (hasLegacyQuery) {
      navigate(cleanPath, { replace: true });
    }
  }, [genreId, sortBy, searchParams, navigate]);
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <SEO
        ...
        description={
          genre
            ? `Stream ${genre.name} TV shows free on JOKER MOVIES. Watch the best ${genre.name.toLowerCase()} series, from binge-worthy dramas to must-watch hits.`
            : 'Stream trending TV shows and series free on JOKER MOVIES. Discover the most popular, top-rated, and newest shows across all genres.'
        }
        url={`https://www.jokermovies.app${buildBrowsePath('tv', genreId)}`}
      />
